...
module.exports = (() => {
    ...
    const { GuildStore, SelectedGuildStore, GuildMemberStore: MemberStore, UserStore, React } = DiscordModules;
...
#permissions-modal .perm-item {
    ...
    <ul class="member-perms \${root} \${rolesList} \${endBodySection}"></ul>
            ...
            this.itemHTML = ({ roleName, permName, roleColor, tagName = "li", dataListItemId, className }) => {
                roleColor ??= "#B9BBBE";

                return Utilities.formatTString((
                    `<${tagName} class="member-perm ${className ?? '${role}'}"
                        ${roleName ? `title="${roleName}"` : ''} 
                        ${dataListItemId ? `data-list-item-id="${dataListItemId}"` : ''} 
                        style="border-color: ${ColorConverter.rgbToAlpha(roleColor, 0.3)}; background-color: ${ColorConverter.rgbToAlpha(roleColor, 0.1)}"
                        data-name="${permName}"
                    >
                        <div class="perm-circle \${roleCircle}" 
                            style="display: none; background-color: ${ColorConverter.rgbToAlpha(roleColor, 1)}"
                        ></div>
                        <div class="name \${roleName}">${permName}</div>
                    </${tagName}>`
                ), DiscordClasses.PopoutRoles);
            };
        ...
        async bindPopouts() {
            const popoutMount = (props) => {
                ...
                for (let r = 0; r < userRoles.length; r++) {
                        ...
                        if (rolesList && roleList.lastElementChild) {
                            rolesList.lastElementChild.insertAdjacentElement(
                                ...
                                DOMTools.createElement(this.itemHTML({
                                    ...
                                    roleName: role.name,
                                    tagName: "div",
                                    dataListItemId: `${rolesList.getAttribute("data-list-id")}___${role.id}`,
                                    className: rolesList.children[0].className
                                }))
                            );
                        }
                    }
                    ...
                    for (const perm in DiscordPerms) {
                        ...
                        if (hasPerm && !memberPerms.querySelector(`[data-name="${permName}"]`)) {
                            memberPerms.prepend(DOMTools.createElement(this.itemHTML({
                                permName, 
                                roleColor: role.colorString,
                                roleName: role.name,
                            })));
                ...
                [...roleList.childNodes.values()].forEach(child => child.addEventListener("click", this.onRoleClicked.bind(this)));
        ...
        async patchGuildContextMenu() {
            ...
            this.contextMenuPatches.push(Patcher.after(GuildContextMenu, "default", (_, [props], retVal) => {
                ...
                const newOne = DCM.buildMenuItem({
                    ...
                    action: () => { this.showModal(this.createModalGuild(props.guild.name, props.guild)); }
                });
        ...
        patchChannelContextMenu() {
            const patch = (_, [{ channel, guild }], retVal) => {
                const original = retVal.props.children[0].props.children;
                const newOne = DCM.buildMenuItem({
                    label: this.strings.contextMenuLabel,
                    action: () => {
        ...
        async patchUserContextMenu() {
            ...
            if (this.promises.state.cancelled) return;
            this.contextMenuPatches.push(Patcher.after(UserContextMenu, "default", (_, [{ user: { id: userId } }], retVal) => {
        ...
        /**
         * 
         * @param {HTMLDivElement} rolesList 
         * @param {MouseEvent & { target: HTMLDivElement }} e 
         * @returns 
         */
        onRoleClicked(e) {
            const element = e.target.hasAttribute("data-list-item-id") ? e.target : e.target.closest("[data-list-item-id]");
            if (!element) return console.log("element not found");

            const roleId = element.getAttribute("data-list-item-id").split("_").pop();
            if (!roleId) return console.log("roleId not found");

            const guild = GuildStore.getGuild(SelectedGuildStore.getGuildId());
            const everyoneRole = guild.roles[Object.keys(guild.roles).find(roleId => guild.roles[roleId].name === "@everyone")];
            const role = guild.roles[roleId];
            if (!role) return console.log("role not found");
            
            const popoutSection = document.querySelector("#permissions-popout");
            const memberPermsHeader = popoutSection.querySelector(".member-perms-header");

            if (!(memberPermsHeader.lastChild instanceof HTMLSpanElement)) {
                this.renderedPermissions = popoutSection.lastElementChild.cloneNode(true);
                memberPermsHeader.append(DOMTools.createElement('<span id="focused-close">&times;</span>'));

                setTimeout(() => document.querySelector("#focused-close").addEventListener("click", () => {
                    memberPermsHeader.lastChild.remove();
                    console.log(this.renderedPermissions);
                    popoutSection.replaceChild(this.renderedPermissions, popoutSection.lastElementChild);
                }), 1000);
            }

            const memberPerms = popoutSection.querySelector(".member-perms");
            memberPerms.innerHTML = "";
            const strings = DiscordModules.Strings;
            ...
            for (const perm in DiscordPerms) {
                const permName = strings[perm] || perm.split("_").map(n => n[0].toUpperCase() + n.slice(1).toLowerCase()).join(" ");
                const hasPerm = (perms & DiscordPerms[perm]) == DiscordPerms[perm];
                const everyonePerm = (everyoneRole.permissions & DiscordPerms[perm]) == DiscordPerms[perm];
                if (hasPerm && (role === everyoneRole || !everyonePerm) && !memberPerms.querySelector(`[data-name="${permName}"]`)) {
                    memberPerms.prepend(DOMTools.createElement(this.itemHTML({
                        permName,
                        roleColor: role.colorString,
                        roleName: role.name,
            ...
            if (!memberPerms.children.length) {
                memberPerms.appendChild(DOMTools.createElement('<span style="color: var(--text-normal)">No difference.</span>'));
            }
        }
