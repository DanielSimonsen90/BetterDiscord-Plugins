/**
 * @name NonObnoxiousProfileEffects
 * @version 1.0.0
 * @author danhosaur
 * @authorLink https://github.com/danhosaur
 * @description Profile effects are cool, but they can be really obnoxious and distracting from the actual profile content. This plugin lowers the opacity of the profile effects (on hover) to make them less distracting.
 * @website https://github.com/DanielSimonsen90/BetterDiscord-Plugins
 * @source https://github.com/DanielSimonsen90/BetterDiscord-Plugins/tree/master/src/NonObnoxiousProfileEffects
**/
...
let meta = {
  "name": "non-obnoxious-profile-effects",
  "version": "1.0.0",
  "author": "danhosaur",
  "description": "Profile effects are cool, but they can be really obnoxious and distracting from the actual profile content. This plugin lowers the opacity of the profile effects (on hover) to make them less distracting."
};
...
const { Select, SingleSelect } =  demangle({
    Select: bySource("renderOptionLabel:", "renderOptionValue:", "popoutWidth:"),
    SingleSelect: bySource((source) => /{value:[a-zA-Z_$],onChange:[a-zA-Z_$]}/.test(source))
}, ["Select"]);

const { TextInput, InputError } = /* @__PURE__ */ demangle({
    TextInput: (target) => target?.defaultProps?.type === "text",
    InputError: bySource("error:", "text-danger")
}, ["TextInput"]);
...
const styles = ".danho-non-obnoxious-profile-effects [class*=profileEffects]:hover {\n  --opacity: 0.2;\n  opacity: var(--opacity);\n}";
...
function SettingsPanel() {
    ...
    return (React.createElement("div", { className: "danho-plugin-settings" },
        React.createElement(Setting, { ...props, setting: "opacity", type: "text" })));
}
