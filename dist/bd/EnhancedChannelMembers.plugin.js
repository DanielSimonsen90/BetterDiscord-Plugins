/**
 * @name EnhancedChannelMembers
 * @version 1.0.0
 * @author danhosaur
 * @authorLink https://github.com/danhosaur
 * @description This plugin allows you to hide activities from the channel members list and also push you to the very top for easy find.
 * @website https://github.com/DanielSimonsen90/BetterDiscord-Plugins
 * @source https://github.com/DanielSimonsen90/BetterDiscord-Plugins/tree/master/src/EnhancedChannelMembers
**/
...
const before = (object, method, callback, options = {}) => patch$1("before", object, method, (cancel, original, context, args) => callback({ cancel, original, context, args }), options);
...
const classNames$1 = /* @__PURE__ */ find((exports) => exports instanceof Object && exports.default === exports && Object.keys(exports).length === 1);
const EventEmitter = /* @__PURE__ */ find((exports) => exports.prototype instanceof Object && Object.prototype.hasOwnProperty.call(exports.prototype, "prependOnceListener"));
...
const Logger$4 = createLogger('bySourceStrings');
function bySourceStrings(...keywords) {
    ...
    if (backupId || lazy || multiple) {
        ...
        Logger$4.debugLog(loggedMessage, { keywords, searchOptions });
    }
    const moduleCallback = (exports, _, id) => {
        ...
        if ((filter && backupId && id.toString() !== backupId) || !filter && id.toString() === backupId)
            Logger$4.debugWarn(`Filter failed for keywords: [${keywords.join(',')}]`, {
                exports,
                internal: {
        ...
        if (backupId && backupId === id.toString())
            Logger$4.debugLog('Found by id', { exports, id });
        return filter;
    };
    const moduleCallbackBoundary = (exports, _, id) => {
        try {
            ...
            if (err instanceof Error && expectedErrorMessages.some(message => err.message.includes(message)))
                return undefined;
            Logger$4.error(`Error in moduleCallback`, err);
        }
    };
    ...
    if (lazy)
        return BdApi.Webpack.waitForModule(moduleCallbackBoundary, {
            ...
            Logger$4.debugLog(`[bySourceStrings] Found lazy module for [${keywords.join(',')}]`, module);
            return module;
        }).catch(err => {
            Logger$4.error(`[bySourceStrings] Error in lazy search`, err);
            return undefined;
        });
}
...
const Logger$3 = createLogger('ErrorBoundary');
class ErrorBoundary extends Component {
    ...
    componentDidCatch(error, errorInfo) {
        Logger$3.error({ error, errorInfo });
    }
    render() {
        if (this.state.error) {
...
const Logger$2 = createLogger('patchComponentBySourceStrings');
function patchComponentBySourceStrings(...args) {
    const jsxModule = byKeys(['jsx']);
    const callback = args.pop();
    const keywords = args.map(keyword => keyword.toString().replace(/\s+/g, ''));
    instead(jsxModule, 'jsx', ({ original, args }) => {
        try {
            const [component, props, children] = args;
            const result = original(...args);
            Logger$2.debugLog({ component, props, children, result });
            if (typeof component === 'function' && keywords.every(keyword => component.toString().includes(keyword))) {
                const patchedResult = callback({
                    result, component,
                    props: Object.assign({}, props, { children: props.children ?? children }),
                });
                return React.createElement(ErrorBoundary, null, patchedResult);
            }
            return result;
        }
        catch (err) {
            Logger$2.error('Error in patching component', err);
        }
...
class DanhoPatcher {
    constructor() {
        this.patchComponentBySourceStrings = patchComponentBySourceStrings;
    }
    after(module, key, callback, options = {}) {
        return after(module, key, callback, options);
    }
    instead(module, key, callback, options = {}) {
        return instead(module, key, callback, options);
    }
    before(module, key, callback, options = {}) {
        return before(module, key, callback, options);
    }
}
const Patcher = Object.assign({}, Patcher$1, new DanhoPatcher());

const Logger$1 = createLogger('ActionsEmitter');
const ActionsEmitter = new class ActionsEmitter extends EventEmitter {
    ...
    on(eventName, listener) {
        ...
        Logger$1.log(`Subscribed to ${eventName}`);
    ...
    off(eventName, listener) {
        ...
        Logger$1.log(`Unsubscribed from ${eventName}`);
    ...
    removeAllListeners(event) {
        ...
        this._events.clear();
        Logger$1.log(`Unsubscribed from all events`);
        return super.removeAllListeners(event);
    }
    emit(eventName, ...args) {
        Logger$1.log(`Emitting ${eventName}`, { args });
        const actionProps = args.shift();
        if (args.length)
            Logger$1.warn(`The following arguments were not used:`, { args });
        const payload = Object.assign({ type: eventName }, actionProps);
        Logger$1.log(`Dispatching ${eventName}`, { payload });
...
const GuildActions = Finder.byKeys(["requestMembers"]);

const SelectedChannelStore = Finder.byName("SelectedChannelStore");

const GuildMemberStore = Finder.byName("GuildMemberStore");

const GuildStore = Finder.byName("GuildStore");

const SelectedGuildStore = Finder.byKeys(["getLastSelectedGuildId"]);

const SortedGuildStore = Finder.byName("SortedGuildStore");

const MessageStore = Finder.byName("MessageStore");

const PresenceStore = Finder.byName("PresenceStore");

const RelationshipStore = Finder.byName("RelationshipStore");

const UserNoteStore = Finder.byKeys(["getNote", "_dispatcher"]);

const UserStore = Finder.byName("UserStore");

const VoiceStore = Finder.byKeys(["getVoiceStateForUser"]);
...
const DiscordRequesterModule = Finder.bySourceStrings("API_ENDPOINT", "API_VERSION", { module: true });
...
function combineModules(...modules) {
    const record = modules.reduce((combined, module, index) => {
        ...
        for (const key in module) {
            ...
            const element = module[key];
            if (typeof element === 'object' && !Array.isArray(element)) {
                combined[prop] = ObjectUtils.combine(combined[prop], element);
            }
            else if (element !== undefined && element !== null && element !== '') {
                combined[prop] = element;
            }
...
const UserUtils = {
    ...
    getUserByUsername: (username) => Object.values(UserStore.getUsers()).find(user => user.username === username),
...
Finder.bySourceStrings("AvatarWithText");
ClassNamesUtils.combineModuleByKeys(["avatarWithText"]);

const UserActivity = Finder.bySourceStrings("CUSTOM_STATUS", "activities", "isBlockedOrIgnored");

const UserAvatar = Finder.bySourceStrings("statusColor", "status", "fromStatus");

const UserPopout = Finder.bySourceStrings("userId", "shouldShowOnHover");
...
function UserListItem(props) {
    const { user, children, className, onClick, openModalOnClick, openPopoutOnClick, showGuildProfile, showActivity, showStatus, roleColorId } = props;
    const roleColor = useMemo(() => roleColorId ? GuildUtils.getGuildRoleWithoutGuildId(roleColorId)?.colorString : undefined, [roleColorId]);
    const handleClick = () => {
        if (onClick)
            onClick(user);
        if (openModalOnClick)
            UserUtils.openModal(user.id, showGuildProfile);
    };
    const Container = ({ children }) => {
        const [showPopout, setShowPopout] = useState(false);
        return openPopoutOnClick ? (React.createElement(UserPopout, { clickTrap: showPopout, shouldShow: showPopout, onRequestClose: () => setShowPopout(false), guildId: GuildUtils.currentId, position: "left", roleId: GuildMemberStore.getMember(GuildUtils.currentId, user.id)?.hoistRoleId, spacing: 16, user: user, shouldShowStatus: showStatus }, props => React.createElement("div", { ...props, onClick: e => {
                props.onClick(e);
                setShowPopout(v => !v);
            }, className: "popout-inner" }, children))) : (React.createElement(React.Fragment, null, children));
    };
    return (React.createElement(Container, null,
        React.createElement("div", { className: classNames("danho-discord-user", onClick && 'clickable', className), onClick: handleClick, onContextMenu: props.onContextMenu },
            React.createElement(AvatarWithText, { avatar: React.createElement(UserAvatar, { status: showStatus ? PresenceStore.getStatus(user.id) : undefined, size: 'SIZE_32', src: user.getAvatarURL(), imageClassName: "danho-discord-user__avatar" }), name: React.createElement(Text, { variant: "text-md/bold", className: "danho-discord-user__displayName", style: { color: roleColor } }, user.globalName ?? user.username), subText: React.createElement(React.Fragment, null,
                    !showActivity
                        ? user.globalName && React.createElement(Text, { variant: "text-sm/normal", className: "danho-discord-user__username" }, user.username)
                        : React.createElement(UserActivity, { activities: PresenceStore.getActivities(user.id, GuildUtils.currentId).concat(PresenceStore.getActivities(user.id)), user: user, animate: true, voiceChannel: VoiceStore.getVoiceStateForUser(user.id)?.channelId, textClassName: ClassNamesUtils.ColorClassNames.colorDefault }),
    ...
    Patcher.patchComponentBySourceStrings('ChannelMembers', "renderRow", ({ component, props }) => {
        ...
        const Wrapper = () => {
            ...
            class DanhoChannelMembersList extends ChannelMembersList {
                ...
                patchedRenderRow(data) {
                    const rowProps = this.getRowProps(data);
                    if (!rowProps)
                        return this.__originalRenderRow(data);
                    if (Settings.current.hideContentInventory && rowProps.type === 'CONTENT_INVENTORY')
                        return null;
                ...
                patchedRenderSection(data) {
                    if (Settings.current.pushYouToTop) {
                        if (data.section === row?.section && !section)
                            setSection(data);
                        if (data.section === 0)
                            return (React.createElement("div", { className: "danho-top-member-header" },
                                this.renderMySection(),
                                this.renderMe()));
                    }
                    if (Settings.current.hideContentInventory && data.section === 0)
                        return null;
                    return this.__originalRenderSection(data);
                }
                renderMySection() {
                    return section
                        ? React.createElement(Text, null, UserUtils.getUsernames(UserUtils.me).shift())
                        : this.__originalRenderSection(section);
                }
                renderMe() {
                    return row
                        ? this.__originalRenderRow(row)
                        : React.createElement(UserListItem, { className: "top-member", user: UserUtils.me, roleColorId: GuildUtils.me.colorRoleId, showStatus: true, showActivity: true, openPopoutOnClick: true });
                }
            }
            return React.createElement(DanhoChannelMembersList, { ...props });
        };
